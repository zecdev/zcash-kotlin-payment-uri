 ...
 *
 * This is the v1 amount type. It carried the name `NonNegativeAmount` up to v1.x; in v2 the
 * name belongs to the new [NonNegativeAmount] value class and this type was renamed
 * `LegacyAmount` (mirroring `zcash-swift-payment-uri`, which renamed the equivalent struct
 * the same way). It stays only until the parser and renderer adopt the v2 type, and it is
 * removed before the v2 release; there is deliberately no `NonNegativeAmount` typealias for
 * the old name, because that name is now taken by the replacement type.
 *
 ...
 */
@Deprecated("Use NonNegativeAmount")
class LegacyAmount {
    internal val value: Long

    /**
     * Initializes a LegacyAmount from a value expressed in Zatoshis.
     *
